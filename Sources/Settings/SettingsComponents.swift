...
// MARK: - 快捷键信息卡片
struct SettingsHotKeyInfoCard: View {
    let title: String
    let icon: String
    let iconColor: Color
    let examples: [String]
...
// MARK: - 功能项组件
struct SettingsFeatureItem: View {
    let icon: String
    let text: String
...
// MARK: - 缩写行组件
struct SettingsAbbreviationRow: View {
    let key: String
    let values: [String]
    let isEditing: Bool
...
// MARK: - 模式设置区块组件
struct SettingsModeSection<Content: View>: View {
    let title: String
    let icon: String
    let iconColor: Color
    let description: String
...
// MARK: - 配置文件内容查看器
struct SettingsConfigContentView: View {
    let content: String
