...
final class StatusMenuManager {
    ...
    private func setupStatusItem(hotkeyDescription: String) {
        ...
        if let button = statusItem?.button {
            let image = NSImage(
                systemSymbolName: "magnifyingglass", accessibilityDescription: "LightLauncher")
    ...
    private func buildMenu() -> NSMenu {
        ...
        menu.addItem(
            NSMenuItem(title: "显示启动器", action: #selector(showLauncherAction), keyEquivalent: ""))
        menu.addItem(NSMenuItem.separator())
        menu.addItem(
            NSMenuItem(title: "设置...", action: #selector(openSettingsAction), keyEquivalent: ","))
        menu.addItem(NSMenuItem.separator())
