...
class PluginConfigManager {
    ...
    ///   - description: 配置描述（可选）
    /// - Returns: 是否设置成功
    func setValue<T>(_ value: T, for key: String, in pluginName: String, description: String? = nil) -> Bool {
    ...
    func getAllConfigNames() -> [String] {
        do {
            let contents = try FileManager.default.contentsOfDirectory(at: configDirectory, 
        ...
        for (key, value) in settingsDict {
            if let configDict = value as? [String: Any],
               ...
               let configValue = configDict["value"] {
                ...
                settings[key] = ConfigValue(type: type, value: configValue, description: description)
            } else {
                // 简单值，推断类型
                let typeString: String
                switch value {
...
extension PluginConfigManager {
    ...
    ///   - pluginName: 插件名称
    ///   - callback: 变化回调
    func registerConfigChangeListener(for pluginName: String, callback: @escaping ConfigChangeCallback) {
